	...
	if(!empty($this->Items)){
		...
		foreach($this->Items as $key => $value){
			...
			foreach ($arrBookID as $keyB => $valueB){
				...
				$picturePath	= PATH_UPLOAD . 'book' . DS . '98x150-' . $arrPicture[$keyB];
				if(file_exists($picturePath)==true){
					$picture	= '<img  width="30" height="45" src="' . URL_UPLOAD . 'book' . DS . '98x150-' . $arrPicture[$keyB].'">';
				}else{
					$picture	= '<img width="30" height="45" src="'.URL_UPLOAD . 'book' . DS . '98x150-default.jpg' .'">';
				}
...
<!-- TITLE -->
<div class="title">
	<span class="title_icon"><img src="<?php echo $imageURL;?>/bullet1.gif"></span>History
</div>
