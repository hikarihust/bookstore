...
class UserModel extends Model{
	...
	public function countItem($arrParam, $option = null){
		...
		if(!empty($arrParam['filter_search'])){
			...
			$query[]	= "WHERE (`username` LIKE $keyword OR `email` LIKE $keyword)";
			$flagWhere 	= true;
		}
		...
		if(isset($arrParam['filter_group_id']) && $arrParam['filter_group_id'] != 'default'){
			if($flagWhere == true){
				...
				$query[]	= "WHERE `group_id` = '" . $arrParam['filter_group_id'] . "'";
				$flagWhere	= true;
			}
		}
	...
	public function listItem($arrParam, $option = null){
		...
		$query[]	= "FROM `$this->table` AS `u` LEFT JOIN `" . TBL_GROUP . "` AS `g`";
		...
		if(isset($arrParam['filter_state']) && $arrParam['filter_state'] != 'default'){
			if($flagWhere == true){
				...
				$query[]	= "WHERE `u`.`status` = '" . $arrParam['filter_state'] . "'";
				$flagWhere	= true;
			}
		}
		...
		if(isset($arrParam['filter_group_id']) && $arrParam['filter_group_id'] != 'default'){
			if($flagWhere == true){
				...
				$query[]	= "WHERE `u`.`group_id` = '" . $arrParam['filter_group_id'] . "'";
				$flagWhere	= true;
			}
		}
