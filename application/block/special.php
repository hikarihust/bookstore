<?php
	$query  = "SELECT `b`.`id`, `b`.`name`, `c`.`name` AS category_name, `b`.`picture`, `b`.`category_id` FROM `".TBL_BOOK."` AS `b` LEFT JOIN ". TBL_CATEGORY ." AS `c` ON `b`.`category_id` = `c`.`id` WHERE `b`.`status`  = 1 AND `b`.`special` = 1 ORDER BY `b`.`ordering` ASC LIMIT 0,2";
